    ...
    return (
      <div className="flex-grow flex items-center justify-center h-full min-h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-500 font-medium text-sm animate-pulse">กำลังตรวจสอบสิทธิ์...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {session?.user?.role === "ADMIN" && (
              <Link href="/dashboard/users" className="block">
                <div className="h-full p-6 bg-blue-50 border border-blue-100 rounded-2xl cursor-pointer hover:bg-blue-100 transition-colors">
            ...
            {(session?.user?.role === "ADMIN" || session?.user?.role === "RH_ST" || session?.user?.role === "SSO") && (
              <Link href="/dashboard/patients" className="block">
                <div className="h-full p-6 bg-green-50 border border-green-100 rounded-2xl cursor-pointer hover:bg-green-100 transition-colors">
            ...
            {(session?.user?.role === "ADMIN" || session?.user?.role === "RH_ST" || session?.user?.role === "SSO" || session?.user?.role === "OSP") && (
              <Link href="/dashboard/careplans" className="block">
                <div className="h-full p-6 bg-yellow-50 border border-yellow-100 rounded-2xl cursor-pointer hover:bg-yellow-100 transition-colors">
            ...
            <Link href="/dashboard/visits" className="block">
              <div className="h-full p-6 bg-purple-50 border border-purple-100 rounded-2xl cursor-pointer hover:bg-purple-100 transition-colors">
            ...
            <Link href="/dashboard/reports" className="block">
              <div className="p-6 bg-indigo-50 border border-indigo-100 rounded-2xl cursor-pointer hover:bg-indigo-100 transition-colors h-full">
